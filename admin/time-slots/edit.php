<?php
/**
 * Admin Time Slots Edit - Time Slot Edit Interface
 * Timetable Management System
 * 
 * Professional interface for admin to edit existing time slots with overlap detection
...
// Get time slot ID to edit
$editSlotId = isset($_GET['id']) ? (int)$_GET['id'] : 0;

if (!$editSlotId) {
    header('Location: index.php?error=' . urlencode('Time Slot ID is required'));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($duration > 14400) { // 4 hours = 14400 seconds
            throw new Exception('Time slot cannot exceed 4 hours.');
        }
        ...
        // Update time slot
        $result = $timeSlotManager->updateTimeSlot($editSlotId, $updateData);
...
try {
    ...
    $timeSlot = $timeSlotManager->getTimeSlotById($editSlotId);
    
    if (!$timeSlot) {
        header('Location: index.php?error=' . urlencode('Time slot not found'));
        exit;
    }
    
} catch (Exception $e) {
    error_log("Edit Time Slot Error: " . $e->getMessage());
    $error_message = "An error occurred while loading the time slot.";
}

// Set page title
$pageTitle = "Edit Time Slot";
?>
...
<head>
    ...
    <style>
        ...
        [data-theme="dark"] .timeslot-info-header {
            background: var(--bg-secondary);
            border-color: var(--border-color);
        }

        [data-theme="light"] .timeslot-info-header {
            background: var(--bg-secondary);
            border-color: var(--border-color);
        }
...
<body data-theme="light">
    ...
    <main class="main-content">
        ...
        <div class="page-header">
            <div class="header-card glass-card fade-in">
                <div class="header-text">
                    <h1 class="page-title">✏️ Edit Time Slot</h1>
                </div>
        ...
        <!-- Time Slot Info Header -->
        <?php if ($timeSlot): ?>
        <div class="timeslot-info-header">
            <div class="timeslot-icon <?= strtolower($timeSlot['slot_type']) ?>">
                <?php
                $icons = [
                ...
                ?>
                <i class="<?= $icons[$timeSlot['slot_type']] ?? 'fas fa-clock' ?>"></i>
            </div>
            <div class="timeslot-details flex-grow-1">
                <h2><?= htmlspecialchars($timeSlot['slot_name']) ?></h2>
                <div class="timeslot-meta">
                    <span><i class="fas fa-calendar-day"></i> <?= ucfirst($timeSlot['day_of_week']) ?></span>
                    <span><i class="fas fa-clock"></i> <?= date('g:i A', strtotime($timeSlot['start_time'])) ?> - <?= date('g:i A', strtotime($timeSlot['end_time'])) ?></span>
                    <span><i class="fas fa-tag"></i> <?= ucfirst($timeSlot['slot_type']) ?></span>
                    <span><i class="fas fa-toggle-<?= $timeSlot['is_active'] ? 'on' : 'off' ?>"></i> <?= $timeSlot['is_active'] ? 'Active' : 'Inactive' ?></span>
                </div>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div class="form-container glass-card slide-up">
            <form method="POST" action="" id="editTimeSlotForm">
                <!-- Day Selection -->
                <div class="form-section">
                    ...
                    <div class="day-selection">
                        ...
                        $selectedDay = $formData['day_of_week'] ?? $timeSlot['day_of_week'] ?? '';
                        ?>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <div class="time-input-group">
                                ...
                                <input type="time" class="form-control" id="start_time" name="start_time" 
                                       value="<?= htmlspecialchars($formData['start_time'] ?? date('H:i', strtotime($timeSlot['start_time'])) ?? '') ?>" 
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <div class="time-input-group">
                                ...
                                <input type="time" class="form-control" id="end_time" name="end_time" 
                                       value="<?= htmlspecialchars($formData['end_time'] ?? date('H:i', strtotime($timeSlot['end_time'])) ?? '') ?>" 
                                       required onchange="calculateDuration()">
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="slot_name" name="slot_name" 
                                   value="<?= htmlspecialchars($formData['slot_name'] ?? $timeSlot['slot_name'] ?? '') ?>" 
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="slot_type" name="slot_type" required>
                                ...
                                <option value="regular" <?= ($formData['slot_type'] ?? $timeSlot['slot_type'] ?? '') === 'regular' ? 'selected' : '' ?>>
                                    Regular Class
                                </option>
                                <option value="break" <?= ($formData['slot_type'] ?? $timeSlot['slot_type'] ?? '') === 'break' ? 'selected' : '' ?>>
                                    Break Time
                                </option>
                                <option value="lunch" <?= ($formData['slot_type'] ?? $timeSlot['slot_type'] ?? '') === 'lunch' ? 'selected' : '' ?>>
                                    Lunch Break
                                </option>
                            </select>
                            <div class="form-text">
                ...
                <div class="form-section">
                    ...
                    <div class="type-selection">
                        <div class="type-card <?= ($formData['slot_type'] ?? $timeSlot['slot_type'] ?? '') === 'regular' ? 'selected' : '' ?>" 
                        ...
                        <div class="type-card <?= ($formData['slot_type'] ?? $timeSlot['slot_type'] ?? '') === 'break' ? 'selected' : '' ?>" 
                        ...
                        <div class="type-card <?= ($formData['slot_type'] ?? $timeSlot['slot_type'] ?? '') === 'lunch' ? 'selected' : '' ?>" 
                ...
                <div class="form-section">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active" 
                               <?= ($formData['is_active'] ?? $timeSlot['is_active'] ?? 1) ? 'checked' : '' ?>>
                        <label class="form-check-label" for="is_active">
                            <strong>Active Time Slot</strong>
                        </label>
                        <div class="form-text mt-1">
                            <i class="fas fa-toggle-on"></i> 
                            Active time slots are available for scheduling. Uncheck to disable this slot.
                        </div>
                    </div>
                </div>
