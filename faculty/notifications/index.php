 ...
 * 
 * Complete notifications view for faculty members
 * Shows all notifications, system announcements, and messages
 */
            ...
            case 'mark_read':
                if ($notificationId > 0) {
                    $db->execute("
                        ...
                        AND (target_user_id = ? OR target_role = 'faculty' OR target_role = 'all')
...
try {
    ...
    if ($facultyInfo) {
        $notificationsData['faculty_info'] = $facultyInfo;
    ...
    $allNotifications = $db->fetchAll("
        SELECT 
            ...
            CASE 
                ...
                WHEN n.target_role = 'faculty' THEN 'faculty'
    ...
    $urgentNotifications = $typesCounts['urgent'] + $typesCounts['error'];
    ...
    <style>
        ...
        * {
        ...
        @media (max-width: 768px) {
            ...
            /* Show list view on mobile and strengthen borders */
            .notifications-grid {
            ...
            /* Header and meta stacking for mobile */
            .list-notification-header {
        ...
        <?php if (!empty($notificationsData['notifications_summary'])): ?>
            <div class="stats-grid">
                ...
                <div class="stat-card glass-card slide-up">
                    ...
                    <div class="stat-number"><?= $notificationsData['notifications_summary']['urgent_count'] ?></div>
        ...
        <div class="notifications-container">
            ...
            <?php if (!empty($notificationsData['all_notifications'])): ?>
                ...
                <div class="notifications-grid" id="gridView">
                    <?php foreach ($notificationsData['all_notifications'] as $notification): ?>
                        <div class="notification-card <?= !$notification['is_read'] ? 'unread' : '' ?> <?= ($notification['type'] === 'urgent' || $notification['type'] === 'error') ? 'urgent' : '' ?>" 
                            ...
                            <div class="notification-status <?= $notification['is_read'] ? 'read' : 'unread' ?> <?= ($notification['type'] === 'urgent' || $notification['type'] === 'error') ? 'urgent' : '' ?>"></div>
                            ...
                            <?php if (!empty($notificationsData['notifications_by_type'][$timeKey])): ?>
                                <div class="time-group">
                                    ...
                                    <?php foreach ($notificationsData['notifications_by_type'][$timeKey] as $notification): ?>
                                        <div class="list-notification <?= !$notification['is_read'] ? 'unread' : '' ?>" 
                                             data-id="<?= $notification['notification_id'] ?>"
                                             data-type="<?= htmlspecialchars($notification['type']) ?>"
                                             data-status="<?= $notification['is_read'] ? 'read' : 'unread' ?>"
                                             data-scope="<?= htmlspecialchars($notification['notification_scope']) ?>">
            ...
            <?php else: ?>
                <div class="empty-notifications">
                    ...
                    <p style="margin-top: 0.5rem;">You'll receive notifications about schedule changes, announcements, and system updates here.</p>
                </div>
            <?php endif; ?>
        </div>
        ...
        function showNotificationDetails(notificationData) {
            ...
            modalBody.innerHTML = `
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        <h6 style="color: var(--primary-color); font-weight: 600; margin-bottom: 1rem;">📋 Notification Details</h6>
                        <div class="mb-3">
                    ...
                    <div class="col-md-6">
                        <h6 style="color: var(--primary-color); font-weight: 600; margin-bottom: 1rem;">👤 Sender Information</h6>
                    ...
                    <div class="col-12">
                        <h6 style="color: var(--primary-color); font-weight: 600; margin-bottom: 1rem;">💬 Message</h6>
...
<?php
/**
 * Helper function for formatting notification priority
...
function formatNotificationPriority($priority) {
    ...
    return $priorities[$priority] ?? '➡️ Normal';
}

/**
 * Helper function to check if notification is recent
 * @param string $created_at
 * @return bool
 */
function isRecentNotification($created_at) {
    return strtotime($created_at) > strtotime('-24 hours');
}

/**
 * Helper function to get notification age in hours
 * @param string $created_at
 * @return int
 */
function getNotificationAgeHours($created_at) {
    return floor((time() - strtotime($created_at)) / 3600);
}
?>