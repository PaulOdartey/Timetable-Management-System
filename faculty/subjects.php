 ...
 * 
 * Complete subjects view for faculty members
 * Shows all assigned subjects with details and student information
 */
...
try {
    ...
    if ($facultyInfo) {
        $subjectsData['faculty_info'] = $facultyInfo;
    ...
    $assignedSubjects = $db->fetchAll("
        SELECT 
            ...
            GROUP_CONCAT(DISTINCT t.section ORDER BY t.section) as sections,
        ...
        WHERE fs.faculty_id = ? 
            AND fs.is_active = 1
            AND s.is_active = 1
        GROUP BY s.subject_id, fs.assignment_id
        ORDER BY s.semester ASC, s.year_level ASC, s.subject_code ASC
    ", [$currentSemester, $currentSemester, $facultyId]);

    $subjectsData['assigned_subjects'] = $assignedSubjects;
    ...
    $totalEnrolledStudents = array_sum(array_column($assignedSubjects, 'enrolled_students'));
    $totalScheduledClasses = array_sum(array_column($assignedSubjects, 'scheduled_classes'));
    $totalCredits = array_sum(array_column($assignedSubjects, 'credits'));
    
    // Get unique semesters taught
    $semestersTaught = count(array_unique(array_column($assignedSubjects, 'semester')));
    
    // Get departments taught in
    $departmentsTaught = count(array_unique(array_filter(array_column($assignedSubjects, 'department_name'))));

    $subjectsData['subjects_summary'] = [
        'total_subjects' => $totalSubjects,
        'total_students' => $totalEnrolledStudents,
        'total_classes' => $totalScheduledClasses,
        'total_credits' => $totalCredits,
        'semesters_taught' => $semestersTaught,
        'departments_taught' => $departmentsTaught
    ];

    $subjectsData['total_students'] = $totalEnrolledStudents;
...
<head>
    ...
    <style>
        ...
        [data-theme="dark"] .subject-footer {
        ...
        * {
        ...
        .subject-stats {
            ...
            justify-content: space-between;
            padding-top: 1rem;
            border-top: 2px solid #cbd5e1; /* stronger light-mode border */
        }
...
<body>
    ...
    <main class="main-content">
            ...
            <div class="header-card glass-card fade-in">
                <div class="d-flex justify-content-between align-items-start">
                    ...
                    <div class="d-flex gap-2">
                        <a href="students.php" class="btn-action btn-outline">
                            👥 View Students
                        </a>
        ...
        <?php if (!empty($subjectsData['subjects_summary'])): ?>
            <div class="stats-grid">
                ...
                <div class="stat-card glass-card slide-up">
                    ...
                    <div class="stat-number"><?= $subjectsData['subjects_summary']['total_students'] ?></div>
                    <div class="stat-label">Total Students</div>
                </div>

                <div class="stat-card glass-card slide-up">
                    <div class="stat-icon classes">🏫</div>
                    <div class="stat-number"><?= $subjectsData['subjects_summary']['total_classes'] ?></div>
        ...
        <div class="subjects-container">
            <div class="subjects-header">
                <h2 class="subjects-title">Assigned Subjects</h2>
            ...
            <?php if (!empty($subjectsData['assigned_subjects'])): ?>
                <!-- Grid View -->
                <div class="subjects-grid" id="gridView">
                    <?php foreach ($subjectsData['assigned_subjects'] as $subject): ?>
                        <div class="subject-card" onclick="showSubjectDetails(<?= htmlspecialchars(json_encode($subject)) ?>)">
                            ...
                            <div class="subject-stats">
                                <div class="stat-group">
                                    <div class="stat-item">
                                        <div class="stat-value"><?= $subject['enrolled_students'] ?></div>
                                        <div class="stat-text">Students</div>
                                    </div>
                                    <div class="stat-item">
                                        <div class="stat-value"><?= $subject['scheduled_classes'] ?></div>
                                        <div class="stat-text">Classes</div>
                                    </div>
                                    <?php if ($subject['sections']): ?>
                                        <div class="stat-item">
                                            <div class="stat-value"><?= count(explode(',', $subject['sections'])) ?></div>
                                    ...
                                    <div class="list-subject">
                                        ...
                                        <div class="list-subject-details">
                                            ...
                                            <div class="detail-item">
                                                ...
                                                <?= $subject['enrolled_students'] ?> Students Enrolled
                                            </div>
                                            <div class="detail-item">
                                                <svg class="detail-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M3 21H21M3 10H21M3 7L12 3L21 7V20H3V7Z" stroke="currentColor" stroke-width="2"/>
                                                </svg>
                                                <?= $subject['scheduled_classes'] ?> Scheduled Classes
                                            </div>
                                            <div class="detail-item">
                                                <svg class="detail-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M19 21V5C19 3.89543 18.1046 3 17 3H7C5.89543 3 5 3.89543 5 5V21L12 18L19 21Z" stroke="currentColor" stroke-width="2"/>
                                                </svg>
                                                Sections: <?= $subject['sections'] ?: 'None' ?>
                                            </div>
            ...
            <?php else: ?>
                <div class="empty-subjects">
                    ...
                    <p style="margin-top: 0.5rem;">Contact the admin to get subject assignments.</p>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="background: var(--bg-primary); border: 2px solid #cbd5e1; border-radius: 16px;">
                ...
                <div class="modal-footer" style="border-top: 2px solid #cbd5e1;">
                    ...
                    <a href="#" class="btn btn-primary" id="viewStudentsBtn">View Students</a>
    ...
    <script>
        ...
        function showSubjectDetails(subjectData) {
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            ...
                            <span style="color: var(--text-secondary);">${subjectData.department_name}</span>
                        </div>
                        ` : ''}
                        <div class="mb-3">
                            <strong>Max Students:</strong><br>
                            <span style="color: var(--text-secondary);">${subjectData.max_students || 'Not set'}</span>
                        </div>
                        <div class="mb-3">
                            <strong>Assigned Date:</strong><br>
                            <span style="color: var(--text-secondary);">${new Date(subjectData.assigned_date).toLocaleDateString()}</span>
                        </div>
                    </div>
                    <div class="col-12">
                        <h6 style="color: var(--primary-color); font-weight: 600; margin-bottom: 1rem;">📊 Statistics</h6>
                        <div class="row g-3">
                            <div class="col-md-3">
                                <div style="background: var(--bg-secondary); padding: 1rem; border-radius: 12px; text-align: center;">
                                    <div style="font-size: 1.5rem; font-weight: 700; color: var(--primary-color);">${subjectData.enrolled_students}</div>
                                    <div style="font-size: 0.875rem; color: var(--text-secondary);">Enrolled Students</div>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div style="background: var(--bg-secondary); padding: 1rem; border-radius: 12px; text-align: center;">
                                    <div style="font-size: 1.5rem; font-weight: 700; color: var(--success-color);">${subjectData.scheduled_classes}</div>
                                    <div style="font-size: 0.875rem; color: var(--text-secondary);">Scheduled Classes</div>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div style="background: var(--bg-secondary); padding: 1rem; border-radius: 12px; text-align: center;">
                                    <div style="font-size: 1.5rem; font-weight: 700; color: var(--warning-color);">${subjectData.sections ? subjectData.sections.split(',').length : 0}</div>
                                    <div style="font-size: 0.875rem; color: var(--text-secondary);">Sections</div>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div style="background: var(--bg-secondary); padding: 1rem; border-radius: 12px; text-align: center;">
                                    <div style="font-size: 1.5rem; font-weight: 700; color: var(--error-color);">${subjectData.max_students ? Math.max(0, (subjectData.max_students - subjectData.enrolled_students)) : 'N/A'}</div>
                                    <div style="font-size: 0.875rem; color: var(--text-secondary);">Available Spots</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    ` : ''}
                    ${subjectData.assignment_notes ? `
                    <div class="col-12">
                        <h6 style="color: var(--primary-color); font-weight: 600; margin-bottom: 1rem;">📋 Assignment Notes</h6>
                        <div style="background: var(--bg-secondary); padding: 1rem; border-radius: 12px;">
                            <div style="font-size: 0.875rem; color: var(--text-secondary); line-height: 1.6;">
                                ${subjectData.assignment_notes}
                            </div>
            ...
            viewStudentsBtn.href = `students.php?subject=${subjectData.subject_id}`;
        ...
        function filterBySemester(semester) {
            ...
            subjects.forEach(subject => {
                const semesterInfo = subject.querySelector('[data-semester]');
                const subjectSemester = semesterInfo ? semesterInfo.dataset.semester : '';
                const visible = semester === 'all' || subjectSemester === semester;
        ...
        document.addEventListener('change', function(e) {
            ...
            } else if (e.target.id === 'semesterFilter') {
                filterBySemester(e.target.value);
            }
        });
    </script>
</body>
...
<?php
/**
 * Helper function for formatting assignment date
 * @param string $date
 * @return string
 */
...
/**
 * Helper function to get subject status color
 * @param int $enrolled
 * @param int $max
 * @return string
 */
function getEnrollmentStatus($enrolled, $max) {
...
/**
 * Helper function to check if subject has prerequisites
 * @param string $prerequisites
 * @return bool
 */
function hasPrerequisites($prerequisites) {
    return !empty(trim($prerequisites));
}
?>